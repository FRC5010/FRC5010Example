...
public class NovaWrapper extends SmartMotorController {
  ...
  /** Nova Conversion. */
  private Conversion m_positionConversion =
      new Conversion(PositionUnit.ROTATIONS, EncoderType.INTERNAL);
  /** Nova Conversion. */
  private Conversion m_velocityConversion =
      new Conversion(VelocityUnit.RADIANS_PER_SEC, EncoderType.INTERNAL);
  ...
  public void synchronizeRelativeEncoder() {
    ...
    //    {
    //      DriverStation.reportWarning(
    //          "[WARNING] NovaWrapper.synchronizeRelativeEncoder() is not supported on
    // ThriftyNova's.",
    //          false);
    //    }
  }
  ...
  public void setEncoderPosition(Angle angle) {
    m_nova.setEncoderPosition(
        m_positionConversion.toMotor(
            angle.times(m_gearing.getMechanismToRotorRatio()).in(Rotations)));
  ...
  public boolean applyConfig(SmartMotorControllerConfig config) {
    ...
    if (m_expoPidController.isEmpty()) {
      if (m_pidController.isEmpty()) {
        ...
        throw new SmartMotorControllerConfigurationException(
            "ProfiledPIDController and PIDController defined",
        ...
        .ifPresent(
            tolerance -> {
              if (config.getLinearClosedLoopControllerUse()) {
                m_pidController.ifPresent(
                    pidController ->
                        pidController.setTolerance(
                            config.convertFromMechanism(tolerance).in(Meters)));
                m_simplePidController.ifPresent(
                    pidController ->
                        pidController.setTolerance(
                            config.convertFromMechanism(tolerance).in(Meters)));
                m_expoPidController.ifPresent(
                    pidController ->
                        pidController.setTolerance(
                            config.convertFromMechanism(tolerance).in(Meters)));
              } else {
    ...
    // Ramp rates
    //    m_nova_config.rampDown = m_nova_config.rampUp =
    // config.getClosedLoopRampRate().in(Seconds);
    ...
    if (config.getExternalEncoder().isPresent() && useExt) {
      ...
      if (config.getZeroOffset().isPresent()) {
        throw new SmartMotorControllerConfigurationException(
            "Zero offset is unavailable for ThriftyNova",
    ...
    if (config.getMaxDiscontinuityPoint().isPresent()
        && !(m_expoPidController.isPresent()
            || m_pidController.isPresent()
            || m_simplePidController.isPresent())) {
      throw new IllegalArgumentException(
        ...
        && config.getMinDiscontinuityPoint().isPresent()) {
  ...
  public double getDutyCycle() {
    return m_simSupplier.isPresent()
        ? m_simSupplier.get().getMechanismStatorVoltage().in(Volts)
            / m_simSupplier.get().getMechanismSupplyVoltage().in(Volts)
        : m_nova.get();
  }
  ...
  public Current getStatorCurrent() {
    return m_simSupplier.isPresent()
        ? m_simSupplier.get().getCurrentDraw()
        : Amps.of(m_nova.getStatorCurrent());
  }
  ...
  public Voltage getVoltage() {
    return m_simSupplier.isPresent()
        ...
        : Volts.of(m_nova.getVoltage() * m_nova.get());
  }
  ...
  public Angle getMechanismPosition() {
    ...
    if (m_config.getUseExternalFeedback() && m_config.getExternalEncoder().isPresent()) {
      ...
      if (externalEncoder == EncoderType.ABS) {
        return Rotations.of(
            m_positionConversion.fromMotor(m_nova.getPositionAbs())
                * m_config.getExternalEncoderGearing().getRotorToMechanismRatio());
      } else if (externalEncoder == EncoderType.QUAD) {
        return Rotations.of(
            m_positionConversion.fromMotor(m_nova.getPositionQuad())
                * m_config.getExternalEncoderGearing().getRotorToMechanismRatio());
      }
    }
  ...
  public void setMotionProfileMaxVelocity(LinearVelocity maxVelocity) {
    if (m_pidController.isPresent()) {
      ...
      ctr.setConstraints(
          new Constraints(maxVelocity.in(MetersPerSecond), ctr.getConstraints().maxAcceleration));
  ...
  public void setMotionProfileMaxAcceleration(LinearAcceleration maxAcceleration) {
    if (m_pidController.isPresent()) {
      ...
      ctr.setConstraints(
          new Constraints(
              ctr.getConstraints().maxVelocity, maxAcceleration.in(MetersPerSecondPerSecond)));
      m_config.withClosedLoopController(ctr);
      m_pidController = Optional.of(ctr);
    }
  }
  ...
  public void setMotionProfileMaxVelocity(AngularVelocity maxVelocity) {
    if (m_pidController.isPresent()) {
      ...
      ctr.setConstraints(
          new Constraints(
              maxVelocity.in(RotationsPerSecond), ctr.getConstraints().maxAcceleration));
      m_config.withClosedLoopController(ctr);
      m_pidController = Optional.of(ctr);
    }
  }
  ...
  public void setMotionProfileMaxAcceleration(AngularAcceleration maxAcceleration) {
    if (m_pidController.isPresent()) {
      ...
      ctr.setConstraints(
          new Constraints(
              ctr.getConstraints().maxVelocity, maxAcceleration.in(RotationsPerSecondPerSecond)));
      m_config.withClosedLoopController(ctr);
      m_pidController = Optional.of(ctr);
    }
  }
