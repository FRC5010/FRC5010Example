...
public class SwerveInputStream implements Supplier<ChassisSpeeds> {
  ...
  public SwerveInputStream driveToPoseEnabled(boolean enabled) {
    ...
    //    driveToPoseXPIDController.ifPresent(profiledPIDController ->
    // profiledPIDController.reset(swervePose.getX()));
    //    driveToPoseYPIDController.ifPresent(profiledPIDController ->
    // profiledPIDController.reset(swervePose.getY()));
    //    driveToPoseOmegaPIDController.ifPresent(profiledPIDController ->
    // profiledPIDController.reset(swervePose.getRotation()
    //
    //             .getRadians()));
    return this;
  }
  ...
  public void resetDriveToPosePIDControllers() {
    ...
    driveToPoseTranslationPIDController.ifPresent(
        pid -> pid.reset(driveToPoseDistance().in(Meters)));
  }
  ...
  private SwerveInputMode findMode() {
    if (driveToPoseEnabled.isPresent()
        && driveToPoseEnabled.get().getAsBoolean()
        && driveToPoseDistance().lte(Centimeters.of(1))
   ...
   *     no pose supplier has been given.
   */
    ...
    switch (newMode) {
      ...
      case AIM -> {
        ...
        Translation2d relativeTrl =
            aimTarget.orElseThrow().relativeTo(swerveDrive.getPose()).getTranslation();
        Rotation2d target =
      ...
      case DRIVE_TO_POSE -> {
        ...
        Vector<N2> robotForwardVec =
            robotPose
                .transformBy(new Transform2d(1, 0, new Rotation2d()))
                .getTranslation()
                .toVector()
                .minus(robotVec);
        Vector<N2> robotLateralVec =
            robotPose
