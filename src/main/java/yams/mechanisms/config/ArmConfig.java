...
public class ArmConfig {
  ...
  public ArmConfig withSmartMotorController(SmartMotorController motorController) {
    if (motor.isPresent()) {
      throw new ArmConfigurationException(
          "Arm already has a SmartMotorController!",
          "Cannot set a new one!",
          ".withSmartMotorController(SmartMotorController)");
    }
    motor = Optional.of(motorController);
    moi.ifPresent(
        moi -> motorController.getConfig().withMomentOfInertia(KilogramSquareMeters.of(moi)));
    ...
    continuousWrapping.ifPresent(
        wrapping -> withWrapping(wrapping.getFirst(), wrapping.getSecond()));
    return this;
  }
   ...
   *
   * @param MOI Moment of Inertia of the {@link yams.mechanisms.positional.Arm}. in {@link
