...
public class SwerveDrive {
  ...
  public SwerveDrive(SwerveDriveConfig config) {
    ...
    var desiredModuleStatesTopic =
        m_telemetry.getDataTable().getStructArrayTopic("states/desired", SwerveModuleState.struct);
    var currentModuleStatesTopic =
        m_telemetry.getDataTable().getStructArrayTopic("states/current", SwerveModuleState.struct);
    ...
    var desiredRobotRelativeChassisSpeedsTopic =
        m_telemetry.getDataTable().getStructTopic("chassis/desired", ChassisSpeeds.struct);
    var fieldRelativeChassisSpeedsTopic =
        m_telemetry.getDataTable().getStructTopic("chassis/field", ChassisSpeeds.struct);
    var currentRobotRelativeChassisSpeedsTopic =
        m_telemetry.getDataTable().getStructTopic("chassis/current", ChassisSpeeds.struct);
  ...
  public void resetOdometry(Pose2d pose) {
    ...
    ChassisSpeeds robotRelativeSpeeds =
        ChassisSpeeds.fromRobotRelativeSpeeds(
            new ChassisSpeeds(0, 0, 0), new Rotation2d(getGyroAngle()));
   ...
   */
  public Command sysIdAzimuth(
      SwerveModule module,
      Voltage maxVoltage,
      Velocity<VoltageUnit> stepVoltage,
      Time testDuration) {
    SmartMotorController smc = module.m_azimuthMotorController;
    SysIdRoutine routine =
        module.m_azimuthMotorController.sysId(maxVoltage, stepVoltage, testDuration);
  ...
  public Command sysIdDrive(
    ...
    Config sysIdConfig =
        m_modules[0].m_dirveMotorController.getSysIdConfig(maxVoltage, stepVoltage, testDuration);
                ...
                log -> {
                  for (var mod : m_modules) {
                    log.motor(mod.m_dirveMotorController.getName())
                        .voltage(mod.m_dirveMotorController.getVoltage())
    ...
    return Commands.print("Starting drive sysId!")
        .andThen(
            Commands.runOnce(
                () -> {
                  SwerveModuleState[] rotaryStates =
                      m_kinematics.toSwerveModuleStates(new ChassisSpeeds(0, 0, 1));
                  for (int i = 0; i < m_modules.length; i++) {
                    m_modules[i].m_azimuthMotorController.setPosition(
                        driveType == DriveSysIdTestType.SPIN
                            ? Radians.of(rotaryStates[i].angle.getRadians())
                            : Rotations.of(0));
                  }
                }))
        .andThen(
            Commands.print("Waiting for wheels to align")
        ...
        .finallyDo(
            () -> {
              for (var mod : m_modules) {
                mod.m_dirveMotorController.startClosedLoopController();
              }
            })
