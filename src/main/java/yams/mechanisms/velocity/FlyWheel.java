...
public class FlyWheel extends SmartVelocityMechanism {
      ...
      if (config.isUsingSpeedometerSimulation() && config.getSpeedometerMaxVelocity().isPresent()) {
        mechanismRoot.append(
            new MechanismLigament2d(
                ...
                270
                    - config.getUpperSoftLimit().orElse(RPM.of(0)).in(RPM)
                        / config.getSpeedometerMaxVelocity().orElse(RPM.of(20000)).in(RPM)
                        * 180,
                6,
        ...
        mechanismRoot.append(
            new MechanismLigament2d(
                ...
                ShooterLength.in(Meters),
                270
                    - config.getLowerSoftLimit().orElse(RPM.of(0)).in(RPM)
                        / config.getSpeedometerMaxVelocity().orElse(RPM.of(20000)).in(RPM)
                        * 180,
                6,
  ...
  public Command run(LinearVelocity speed) {
    return setSpeed(m_config.getAngularVelocity(speed))
        .withName(m_subsystem.getName() + " RunSpeed");
  }
  ...
  public Trigger max() {
    ...
    throw new FlyWheelConfigurationException(
        "FlyWheel upper soft limit is empty",
  ...
  public Command sysId(Voltage maximumVoltage, Velocity<VoltageUnit> step, Time duration) {
    ...
    return group
        .withName(m_subsystem.getName() + " SysId")
  ...
  public void simIterate() {
    if (m_dcmotorSim.isPresent() && m_smc.getSimSupplier().isPresent()) {
      ...
      RoboRioSim.setVInVoltage(
          BatterySim.calculateDefaultBatteryLoadedVoltage(m_dcmotorSim.get().getCurrentDrawAmps()));
      visualizationUpdate();
    }
  }
