...
public class FlyWheelConfig {
   ...
   * FlyWheel configuration class.
   *
   * @implNote Required to call {@link #withSmartMotorController(SmartMotorController)} before this
   *     is used with an {@link FlyWheel}
   */
        ...
        Optional.ofNullable(
            maxVelocity.orElse(RPM.of(0)).abs(RPM) > (minVelocity.orElse(RPM.of(0))).abs(RPM)
                ...
                : RPM.of(
                    minVelocity
                        .orElse(RPM.of(0))
                        .abs(RPM))); // The orElse here is just to deal with the Optional, it will
    // likely never be 0
    return this;
  }
   ...
   */
  @Deprecated
  public FlyWheelConfig withTelemetry(
  ...
  public double getMOI() {
    ...
    if (diameter.isPresent() && weight.isPresent()) {
      return SingleJointedArmSim.estimateMOI(
          diameter.get().in(Units.Meters), weight.get().in(Units.Kilograms));
    }
    throw new FlyWheelConfigurationException(
