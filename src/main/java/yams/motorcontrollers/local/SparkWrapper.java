...
public class SparkWrapper extends SmartMotorController {
  ...
  public void setVelocity(AngularVelocity angularVelocity) {
    ...
    if (m_expoPidController.isEmpty() && angularVelocity != null) {
      m_sparkPidController.setSetpoint(
          angularVelocity.in(RotationsPerSecond), m_velocityControlType, m_closedLoopSlot);
    }
    m_looseFollowers.ifPresent(
        smcs -> {
          for (var f : smcs) {
  ...
  public boolean applyConfig(SmartMotorControllerConfig config) {
    ...
    m_sparkBaseConfig
        .openLoopRampRate(config.getOpenLoopRampRate().in(Seconds))
    ...
    config
        ...
        .ifPresent(
            pidController -> {
              ...
              if (config.getLinearClosedLoopControllerUse()) {
                maxVel =
                    config.convertToMechanism(MetersPerSecond.of(maxVel)).in(RotationsPerSecond);
                maxAccel =
                    config
                        .convertToMechanism(MetersPerSecondPerSecond.of(maxAccel))
    ...
    config
        ...
        .ifPresent(
            ff -> {
              m_sparkBaseConfig.closedLoop.feedForward.kS(ff.getKs()).kV(ff.getKv()).kA(ff.getKa());
            });
      ...
      for (Pair<Object, Boolean> follower : config.getFollowers().get()) {
        if (follower.getFirst() instanceof SparkMax) {
          ((SparkMax) follower.getFirst())
              .configure(
                  new SparkMaxConfig().follow(m_spark, follower.getSecond()),
                  ResetMode.kNoResetSafeParameters,
                  PersistMode.kPersistParameters);
          ...
          ((SparkFlex) follower.getFirst())
              .configure(
                  new SparkFlexConfig().follow(m_spark, follower.getSecond()),
                  ResetMode.kNoResetSafeParameters,
                  PersistMode.kPersistParameters);
  ...
  public double getDutyCycle() {
    return m_spark
        .getAppliedOutput(); /* m_simSupplier.map(simSupplier -> simSupplier.getMechanismStatorVoltage().in(Volts) /
  ...
  public void setMotorInverted(boolean inverted) {
    ...
    m_spark.configureAsync(
        m_sparkBaseConfig, ResetMode.kNoResetSafeParameters, PersistMode.kPersistParameters);
  }
