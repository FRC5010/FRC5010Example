...
public class TalonFXWrapper extends SmartMotorController {
  ...
  public boolean applyConfig(SmartMotorControllerConfig config) {
    ...
    if (config.getClosedLoopController().isPresent()
        && config.getSimpleClosedLoopController().isPresent()) {
      throw new SmartMotorControllerConfigurationException(
    ...
    if (config.getExponentiallyProfiledClosedLoopController().isPresent()) {
      ExponentialProfilePIDController controller =
          config.getExponentiallyProfiledClosedLoopController().get();
      ...
      //      {
      //        //m_talonConfig.Slot0.kP =
      // config.convertToMechanism(Meters.of(controller.getP())).in(Rotations);
      //        //m_talonConfig.Slot0.kI =
      // config.convertToMechanism(Meters.of(controller.getI())).in(Rotations);
      //        //m_talonConfig.Slot0.kD =
      // config.convertToMechanism(Meters.of(controller.getD())).in(Rotations);
      //
      //      } else
      //      {
      //
      ...
      if (config.getExternalEncoder().get() instanceof CANcoder encoder) {
        ...
        if (config.getMaxDiscontinuityPoint().isPresent()) {
          cfg.MagnetSensor.withAbsoluteSensorDiscontinuityPoint(
              config.getMaxDiscontinuityPoint().get());
        }
        configurator.apply(cfg);
  ...
  public void setMeasurementUpperLimit(Distance upperLimit) {
    if (m_config.getMechanismCircumference().isPresent()
      ...
      m_talonConfig.SoftwareLimitSwitch.withForwardSoftLimitThreshold(
          m_config.convertToMechanism(upperLimit));
      forceConfigApply();
    }
  }
