...
public class PivotConfig {
  ...
  /**
   * Soft limits of the pivot motor {@link SmartMotorController} closed loop controller.
   * (LowerLimit, UpperLimit)
   */
  private Optional<Pair<Angle, Angle>> softLimits = Optional.empty();
  /**
   * Continuous wrapping of the pivot {@link SmartMotorController} closed loop controller. (Min,
   * Max)
   */
   ...
   * Pivot Configuration class
   *
   * @implNote Required call to {@link #withSmartMotorController(SmartMotorController)} before
   *     usage.
   */
  public PivotConfig() {
    mechanismPositionConfig.withMovementPlane(Plane.XY);
   ...
   *
   * @param motorController {@link SmartMotorController} for the {@link
   *     yams.mechanisms.positional.Pivot}.
  ...
  public PivotConfig withSmartMotorController(SmartMotorController motorController) {
    if (motor.isPresent()) {
      throw new PivotConfigurationException(
          "Pivot SmartMotorController already set!",
    ...
    continuousWrapping.ifPresent(
        continuousWrapping ->
            withWrapping(continuousWrapping.getFirst(), continuousWrapping.getSecond()));
    return this;
  }
   ...
   *
   * @param MOI Moment of Inertia of the {@link yams.mechanisms.positional.Pivot}. in {@link
   *     edu.wpi.first.units.Units#KilogramSquareMeters}
   * @return {@link PivotConfig} for chaining.
   * @implNote Please use {@link #withMOI(MomentOfInertia)} instead. Default unit is
