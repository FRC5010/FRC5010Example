...
/**
 * Exponential profile PID controller. Similar to {@link PIDController} or {@link
 * edu.wpi.first.math.controller.ProfiledPIDController}, but uses an {@link ExponentialProfile}
 */
public class ExponentialProfilePIDController {
   ...
   * @param gearing {@link MechanismGearing} of the arm from the rotor to the drum. {@code
  ...
  public static ExponentialProfile.Constraints createFlywheelConstraints(
    ...
    // A.in(RotationsPerSecond), B.in(RotationsPerSecondPerSecond));
    ////    return ExponentialProfile.Constraints.fromCharacteristics(maxVolts.in(Volts), -A/B,
    // 1.0/B);
  }
  ...
  /**
   * Reset the PID and profile with the given postion and velocity as the measured position and
   * velocity.
   *
