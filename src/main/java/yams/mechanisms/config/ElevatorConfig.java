...
public class ElevatorConfig {
  ...
  /**
   * Soft limits of the {@link SmartMotorController} closed loop controller. Can be exceeded.
   ...
   * Elevator Configuration class
   *
   * @implNote You are REQUIRED to call {@link #withSmartMotorController(SmartMotorController)}
   *     before this is used with an {@link yams.mechanisms.positional.Elevator}
   */
  ...
  public ElevatorConfig withSmartMotorController(SmartMotorController motorController) {
    ...
    drumCircumference.ifPresent(
        drumRadius -> motor.get().getConfig().withMechanismCircumference(drumRadius));
    stages.ifPresent(this::withCascadingElevatorStages);
    startingHeight.ifPresent(this::withStartingHeight);
    softLimits.ifPresent(
        softLimits -> withSoftLimits(softLimits.getFirst(), softLimits.getSecond()));
    return this;
  }
  ...
  public ElevatorConfig withDrumRadius(Distance drumRadius) {
    ...
    motor.ifPresent(
        motor -> motor.getConfig().withMechanismCircumference(drumRadius.times(2 * Math.PI)));
    return this;
  }

  /**
   * Set the {@link yams.mechanisms.positional.Elevator} drum radius via the chain pitch (.25in or
   * .35in) and teeth count.
   *
  ...
  public ElevatorConfig withCascadingElevatorStages(int stages) {
    ...
    motor.ifPresent(
        motor -> motor.getConfig().withGearing(motor.getConfig().getGearing().div(stages)));
    return this;
  }
  ...
  public Optional<Distance> getStartingHeight() {
    return motor.orElseThrow().getConfig().getStartingPosition().isPresent()
        ? Optional.of(
            motor
                .orElseThrow()
                .getConfig()
                .convertFromMechanism(motor.orElseThrow().getConfig().getStartingPosition().get()))
        : Optional.empty();
  }
